...
class ReassignMultipleTasksCommandHandler implements CommandHandler 
{   
	use DispatchesJobs;
	/**
	...
	public function handle($command)
	{
		...
		foreach ($command->list as $value) {
			$value = (object) $value;
			
	        if($this->taskValidator->isTaskCompleted($value->taskId)) {
	            throw new SeException("Task is already completed", 422, 786125);
	        }

			try{
				\DB::beginTransaction();
				$task = $this->taskRepository->getTaskById($value->taskId);

	            if($task->assignee->email === $value->assignee) {
				...
				$value->assignee = $this->taskHelper->getAssigneeDetails($value->assignee);
				$task = $this->taskRepository->reassignTask($value);
				\DB::commit();
			} catch(Exception $e) {
