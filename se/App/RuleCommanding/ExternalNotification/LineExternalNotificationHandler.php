...
*/
class LineExternalNotificationHandler extends Notifier
{
    ...
    public function createNewLine($line, $actor)
    {  
        $valDefinition = $this->updateCollection($line);
        $valDefinition->link = $this->getHostUrl().'line/'.$valDefinition->id;
        $valDefinition->actorName = $actor->display_name;
        $subject = $actor->display_name.'('.$actor->email.') has created a line '.$valDefinition->name;
        $defaultReceiver = ['sales_representative_id', 'product_development_lead_id'];
        $this->processMail($valDefinition, $actor, $defaultReceiver, 'emails.line.CreateNewLine', $subject);
        return $line;
    }
